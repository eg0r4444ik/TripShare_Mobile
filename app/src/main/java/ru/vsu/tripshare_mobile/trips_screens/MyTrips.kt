...
@Composable
fun MyTrips(trips: List<TripParticipantModel>, person: UserModel, navController: NavController) {
    ...
    Column(
        ...
        LazyColumn(
            ...
            itemsIndexed(trips) { _, item ->
                TripCard(trip = item.trip, cost = item.cost, person = person, navController = navController)
            }
        }
    }
}